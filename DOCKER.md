...
From the directory that contains **`docker-compose.registry.yml`** and **`nginx/`**:
...
docker compose -f docker-compose.registry.yml exec -T postgres psql -U payload -d journalism -c "DELETE FROM payload_migrations WHERE batch = -1;"
```
...
```bash
# Edit docker-compose.registry.yml to add this under the postgres service temporarily:
#   ports:
#     - '127.0.0.1:5433:5432'
docker compose -f docker-compose.registry.yml up -d postgres
```
